...
import {Poseidon2} from "../src/bn254/Poseidon2.sol";
...
contract Poseidon2Test is Test {
    ...
    function setUp() public {
        ...
        // Deploy bytecode at library constant addresses for testing
        vm.etch(Poseidon2.YUL, address(poseidon2Yul).code);
        vm.etch(Poseidon2.HUFF, address(poseidon2Huff).code);
    }

    // ============================================================
    // Test Vectors for hash_1
    // ============================================================

    function test_hash_1_vectors() public view {
        // Test vector pairs: [input, expected_output]
        uint256[2][5] memory vectors = [
            [uint256(0x0), uint256(0x2710144414c3a5f2354f4c08d52ed655b9fe253b4bf12cb9ad3de693d9b1db11)],
            [
                uint256(0x19f3d19e5dd8b29a42cea4f71ebc6b12a42a5edbafbcb89cf0cddf0995d44e7f),
                uint256(0x2e1874598412a3b19f824ff246a1949a38b365bcdd58807eedb9206288820232)
            ],
            [
                uint256(0x0049c16ff91dacf0cb573751342f7bfa0042819e3f15fce57d61339f6340b0c1),
                uint256(0x011b5e5c76aedd3a361d2c72c711425f5709988da3c226bb8536691d81b190ac)
            ],
            [
                uint256(0x02713077725e5498d596be781be4c9a7353dbfe70ff10dc17702e66d0b5d388c),
                uint256(0x2ee1f0fc41b250f0e26dbaa1e9dc0d8e27e9354baf3c11eca131994c119f5651)
            ],
            [
                uint256(0x2a84a08a631f4c391b02f4519720881a80c25eb6ba3b59b2ca8f3c0e22ebeebc),
                uint256(0x0cf97e83eb7aa42f60e85095836d3b0afe3e88cc5046e1bae7318a64a0d32fd5)
            ]
        ];
        ...
        for (uint256 i = 0; i < vectors.length; i++) {
            ...
            assertEq(poseidon2.hash_1(input.toField()).toUint256(), expected, "Solidity hash_1 failed");
            assertEq(poseidon2Yul.hash_1(input), expected, "Yul hash_1 failed");
            assertEq(LibPoseidon2Yul.hash_1(input), expected, "YulLib hash_1 failed");
            assertEq(poseidon2Huff.hash_1(input), expected, "Huff hash_1 failed");
            // Test library wrappers
            assertEq(Poseidon2.hash_1(input), expected, "Lib hash_1 failed");
            assertEq(Poseidon2.hash_1_huff_unchecked(input), expected, "Lib hash_1_huff failed");
        }
    }

    // ============================================================
    // Test Vectors for hash_2
    // ============================================================

    function test_hash_2_vectors() public view {
        // Test vector tuples: [input1, input2, expected_output]
        uint256[3][5] memory vectors = [
            [uint256(0x0), uint256(0x0), uint256(0x0b63a53787021a4a962a452c2921b3663aff1ffd8d5510540f8e659e782956f1)],
            [
                uint256(0x1762d324c2db6a912e607fd09664aaa02dfe45b90711c0dae9627d62a4207788),
                uint256(0x1047bd52da536f6bdd26dfe642d25d9092c458e64a78211298648e81414cbf35),
                uint256(0x303cacb84a267e5f3f46914fd3262dcaa212930c27a2f9de22c080dd9857be35)
            ],
            [
                uint256(0x0a529bb6bbbf25ed33a47a4637dc70eb469a29893047482866748ae7f3a5afe1),
                uint256(0x1f5189751e18cf788deb219bdb95461e86ca08a882e4af5f8a6ec478e1ec73b4),
                uint256(0x1f71000626ba87581561f94a17f7b9962be8f1aa8d0c69c6f95348c9ddffe542)
            ],
            [
                uint256(0x14ba77172ab2278bdf5a087ca0bd400e936bafe6dfc092c4e7a1b0950f1b6dbe),
                uint256(0x195c41f12d4fbac5e194c201536f3094541e73bf27d9f2413f09e731b3838733),
                uint256(0x06b53c119381e6ccee8e3ac9845970ba96befbce39606fad3686a6e31ac7761e)
            ],
            [
                uint256(0x2079041f0d6becd26db3ec659c54f60464243d86c3982978f1217a5f1413ed3a),
                uint256(0x08146641a4e30689442ecd270a7efef725bdb3036bf3d837dff683161a455de1),
                uint256(0x07512888968a4cfc7e5da7537c7f4448bf04d3679e711c3f16ca86351b0d1e6b)
            ]
        ];

        for (uint256 i = 0; i < vectors.length; i++) {
            uint256 input1 = vectors[i][0];
            uint256 input2 = vectors[i][1];
            uint256 expected = vectors[i][2];

            // Test all implementations
            assertEq(
                poseidon2.hash_2(input1.toField(), input2.toField()).toUint256(), expected, "Solidity hash_2 failed"
            );
            assertEq(poseidon2Yul.hash_2(input1, input2), expected, "Yul hash_2 failed");
            assertEq(LibPoseidon2Yul.hash_2(input1, input2), expected, "YulLib hash_2 failed");
            assertEq(poseidon2Huff.hash_2(input1, input2), expected, "Huff hash_2 failed");
            // Test library wrappers
            assertEq(Poseidon2.hash_2(input1, input2), expected, "Lib hash_2 failed");
            assertEq(Poseidon2.hash_2_huff_unchecked(input1, input2), expected, "Lib hash_2_huff failed");
        }
    }
    ...
    function test_hash_3_vectors() public view {
        // Test vector tuples: [input1, input2, input3, expected_output]
        uint256[4][5] memory vectors = [
            [
                uint256(0x0),
                uint256(0x0),
                uint256(0x0),
                uint256(0x2a5de47ed300af27b706aaa14762fc468f5cfc16cd8116eb6b09b0f2643ca2b9)
            ],
            [
                uint256(0x300ced31bf248a1a2d4ea02b5e9f302a9e34df3c2109d5f1046ee9f59de6f6f1),
            ...
            [
                uint256(0x21c6e3a949ec9c622e7e062efab3d6e1d5ee62763f99366627de157e26d179b7),
                uint256(0x1369519755b97bf50d10be283f853c5607ed1308f8235cd34c915783cbf7c70d),
                uint256(0x00c632d6fe8be14eddb11aee10b63e017e7d1f1a275d01e9d215f4135e950e7d),
                uint256(0x0d72d2b806a47af41117f7de2147908eb0496f99c0f58fbd6f5e778174132451)
            ],
            [
                uint256(0x26001a9f75eddc947cfc2e7a5524a7fb32d226404468168d58ff2f918e027015),
                uint256(0x0080918fd6c6f3867a46a0260e2cc2bc6b0b6bcb4528d33ba2bb57555a12929a),
                uint256(0x05217f0ada44bebd06d9fd34358fe7213446bdce940d943916acb59cabdc0001),
                uint256(0x2015f0a35636e589ae36e65f00b8f8c8a0cab01dcd080c23c8e2274283a57c85)
            ],
            [
                uint256(0x2a7efeb117f6894cfa21375d4496249c43eda35b5d5d267b7154413fdaad44ea),
                uint256(0x0bb48b46f770205618e2350c2f9c294524425763fb7ef6156c33c5bc2b8c1cbf),
                uint256(0x0206c176b0c66212fd12a580a8dd38689ae29dfa15933ba1982f5d30ed0c2ea1),
                uint256(0x1c93b16c25fa1e7b4933425b32876fa97c2013d44214792231634d3c96b6cc3f)
            ]
        ];
        ...
        for (uint256 i = 0; i < vectors.length; i++) {
            uint256 input1 = vectors[i][0];
            uint256 input2 = vectors[i][1];
            uint256 input3 = vectors[i][2];
            uint256 expected = vectors[i][3];

            // Test all implementations
            assertEq(
                poseidon2.hash_3(input1.toField(), input2.toField(), input3.toField()).toUint256(),
                expected,
                "Solidity hash_3 failed"
            );
            assertEq(poseidon2Yul.hash_3(input1, input2, input3), expected, "Yul hash_3 failed");
            assertEq(LibPoseidon2Yul.hash_3(input1, input2, input3), expected, "YulLib hash_3 failed");
            assertEq(poseidon2Huff.hash_3(input1, input2, input3), expected, "Huff hash_3 failed");
            // Test library wrappers
            assertEq(Poseidon2.hash_3(input1, input2, input3), expected, "Lib hash_3 failed");
            assertEq(Poseidon2.hash_3_huff_unchecked(input1, input2, input3), expected, "Lib hash_3_huff failed");
        }
    }
    ...
    function test_hash_through_library() public pure {
        LibPoseidon2.Constants memory _poseidon = LibPoseidon2.load();
        assertEq(
            _poseidon.hash_1(uint256(0x0).toField()).toUint256(),
    ...
    function test_hash_var_len() public view {
        // Test variable length hashing with 1-10 inputs
        _testVarLen1();
        _testVarLen2();
        _testVarLen3();
        _testVarLen4();
    ...
    function testFuzz_hash_1_all_implementations(uint256 x) public view {
        ...
        uint256 libResult = Poseidon2.hash_1(x);
        uint256 libHuffResult = Poseidon2.hash_1_huff_unchecked(x);
    ...
    function testFuzz_hash_2_all_implementations(uint256 x, uint256 y) public view {
        ...
        uint256 libResult = Poseidon2.hash_2(x, y);
        uint256 libHuffResult = Poseidon2.hash_2_huff_unchecked(x, y);
    ...
    function testFuzz_hash_3_all_implementations(uint256 x, uint256 y, uint256 z) public view {
        ...
        uint256 libResult = Poseidon2.hash_3(x, y, z);
        uint256 libHuffResult = Poseidon2.hash_3_huff_unchecked(x, y, z);
